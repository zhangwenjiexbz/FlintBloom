...
class SQLiteAdapter(BaseDatabaseAdapter):
    ...
    def get_database_info(self) -> Dict[str, Any]:
        ...
        result = self.session.execute(text("SELECT sqlite_version()"))
        ...
        # Get database size
        result = self.session.execute(text("PRAGMA page_count"))
        page_count = result.scalar()
        result = self.session.execute(text("PRAGMA page_size"))
