...
from app.modules.realtime.collector import RealtimeCollector


class FlintBloomCallbackHandler(BaseCallbackHandler):
    """
    Custom callback handler for LangChain/LangGraph with dynamic thread_id support

    Supports multiple ways to specify thread_id:

    1. Static thread_id (backward compatible):
        callback = FlintBloomCallbackHandler(thread_id="my-thread")

    2. Auto-detect from LangGraph config:
    ...
    3. Custom resolver function:
        def get_thread_id(metadata):
            return f"user-{metadata.get('user_id')}"
    ...
    def __init__(
        ...
        Args:
            ...
            collector: Real-time data collector instance
        ...
        self.collector = collector or RealtimeCollector()
